...
mdl = TreeBagger(200, trainData, trainLabel,...
    "PredictorNames", vars, 'Method','classification',...
    'OOBPredictorImportance', 'on');
mdlLoss = oobError(mdl, 'mode', 'ensemble');
fprintf("model loss rate is: %.4f \n", mdlLoss);
[mdlPred, mdlScore] = predict(mdl, testData);


[~,order] = sort(mdl.OOBPermutedVarDeltaError);  % sort the metrics
figure
barh(mdl.OOBPermutedVarDeltaError(order))        % horizontal bar chart
title('Feature Importance Metric')
ax = gca; ax.YTickLabel = vars(order);          % variable names as labels


% evaluation
cfm = confusionmat(testLabel, string(mdlPred)); % confusion matrix
cvAccuracy = sum(cfm(logical(eye(2))))/length(mdlPred);     % compute accuracy

figure;
confusionchart(testLabel, string(mdlPred));
title("rf");
