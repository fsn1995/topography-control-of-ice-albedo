...
#%%
basinpoly = gpd.read_file("shp/GrISBasinDissolved.shp").to_crs("EPSG:4326")
basinpoly.plot()
...
# # %% Some experiments with other topograhpic data
...
# df = pd.read_csv("/data/shunan/data/topography/surfaceRoughness.csv")

# #%%
# basinpoly = gpd.read_file("src/shp/GrISBasinDissolved.shp").to_crs("EPSG:4326")
# basinpoly.plot()
...
# df.to_csv("/data/shunan/data/topography/roughnessPostProcessed.csv", index=False, mode="w")

# #%%
# '''
# join surface roughtness and albedo
# '''
# #%%
# dfdem = pd.read_csv("/data/shunan/data/topography/roughnessPostProcessed.csv")
# dfalbedo = pd.read_csv("/data/shunan/data/topography/albedo.csv")
# dfdem["datetime"] = pd.to_datetime(dfdem.time_end, unit="ms")
# dfalbedo["datetime"] = pd.to_datetime(dfalbedo.time, unit="ms")
...
# randomPoints = gpd.read_file("src/shp/randamSample.shp")
...
#     if i == 0:
#         dfmerge.to_csv("/data/shunan/data/topography/topomergeRoughness.csv", 
#                         mode="w", index=False, header=True)
#     else:
#         dfmerge.to_csv("/data/shunan/data/topography/topomergeRoughness.csv", 
#                         mode="a", index=False, header=False)
...
# #%% estimate duration of bare ice
# df = pd.read_csv("/data/shunan/data/topography/topomergeRoughness.csv")
# df["datetime"] = pd.to_datetime(df.time_x, unit="ms")
# df["year"] = df.datetime.dt.year
# df["month"] = df.datetime.dt.month
# df["duration"] = np.nan
...
#     dfsub = dfsub.drop(columns = ['datetime', 'year', 'month'])
#     if i == 0:
#         dfsub.to_csv("/data/shunan/data/topography/topodataRoughness.csv", 
#                         mode="w", index=False, header=True)   
#     else:
#         dfsub.to_csv("/data/shunan/data/topography/topodataRoughness.csv", 
#                         mode="a", index=False, header=False)  
...
# #%% keep bare ice surface only
# df = pd.read_csv("/data/shunan/data/topography/topodataRoughness.csv")
# df["datetime"] = pd.to_datetime(df.time_x, unit="ms")
# df["year"] = df.datetime.dt.year
# df["month"] = df.datetime.dt.month
...
#     dfbasin = df[df.basin == i].groupby(["id", "year", "month"]).mean()
#     dfbasin["basin"] = i
#     dfbasin.to_csv("/data/shunan/data/topography/basinRoughness/" + i + ".csv", mode="w")
    
#     dfbasin = df[df.basin == i]
#     monthindex = (dfbasin.month>6) & (dfbasin.month<9)
#     dfbasin = dfbasin[monthindex].groupby(["id", "year"]).mean()
#     dfbasin["basin"] = i
#     dfbasin.to_csv("/data/shunan/data/topography/basinRoughness/" + i + "_annual.csv", mode="w")
